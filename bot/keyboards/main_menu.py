...
def main_menu_kb(user: User) -> InlineKeyboardMarkup:
    ...
    b.button(text="📉 Проскальзывание", callback_data="menu:slippage")
...
def status_text(user: User) -> str:
    ...
    adv = getattr(user, "max_slippage_percent", None)
    fav = getattr(user, "max_favorable_slippage_percent", None)
    slip = (
        f"{adv}%" if adv is not None else "по умолч."
    ) + " / " + (
        f"{fav}%" if fav is not None else "по умолч."
    )
    return (
        ...
        f"Ставка: {mode}\n"
        f"TP: {tp} | SL: {sl}\n"
        f"Проскальзывание (хуже/лучше): {slip}"
    )