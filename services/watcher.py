...
class WalletWatcher:
    ...
    async def _dispatch(
        ...
        for t in reversed(trades):  # от старых к новым
            ...
            if skip_reason == "до Старта":
                # Сделки, сделанные до нажатия Старта, — это обычная
                # история кошелька. При первом запуске их сотни, и
                # WARNING на каждую засорял лог (у активного кошелька —
                # десятки строк подряд), не сообщая ничего полезного.
                logger.debug(
                    f"user={user_id}: {t.tx_hash[:18]}... до Старта"
                )
            elif skip_reason:
                # WARNING: остальные пропуски — это то, что пользователь
                # замечает как "бот не видит ставки".
                logger.warning(
