...
class Settings(BaseSettings):
    ...
    # Пауза стоп-лосса после полной очистки стакана, в секундах.
    # На спортивных рынках Polymarket снимает все заявки в момент начала
    # матча; первые появившиеся после этого заявки часто грабительские
    # (0.01 в расчёте на панику). Стоп ждёт, пока рынок наполнится.
    book_clear_grace_seconds: int = 30
    ...
    # WebSocket-адрес Polygon у Alchemy:
    #   wss://polygon-mainnet.g.alchemy.com/v2/<КЛЮЧ>
    # Пусто — отслеживание по блокчейну выключено, работает только Data API.
    alchemy_ws_url: str = ""
    # shadow — только наблюдение и замер отставания Data API (безопасно)
    # live   — копировать по событиям из блокчейна
    # off    — выключено
    onchain_mode: str = "shadow"
