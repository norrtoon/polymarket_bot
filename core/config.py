...
class Settings(BaseSettings):
    ...
    # Окно схлопывания транзакций ОДНОГО ордера трейдера. Один ордер
    # Polymarket проводит несколькими транзакциями подряд (в пределах
    # 1-3 секунд). Окно должно покрывать это, но быть заметно КОРОЧЕ
    # интервала, через который трейдер реально перезаходит в рынок —
    # иначе настоящие перезаходы будут проглатываться. 15с — компромисс.
    copy_dedup_window_seconds: int = 15
    ...
    # 0 = проверка проскальзывания ОТКЛЮЧЕНА (копируем по любой цене).
    # Стакан при этом всё равно запрашивается: из него берутся
    # минимальный размер ордера и цена для limit_price.
    max_slippage_percent: float = 0.0
    # Отклонение В нашу пользу. Лимит мягче: покупать дешевле трейдера
    # выгодно. Но скачок в разы означает, что рынок переоценил исход —
    # это уже не та сделка, которую совершил трейдер.
    max_favorable_slippage_percent: float = 0.0
